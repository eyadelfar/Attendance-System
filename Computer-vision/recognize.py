        ...
        while True:
            ...
            if self.recognition_counts:
                ...
                for student_id, count in self.recognition_counts.items():
                    ...
                    if result:
                        student_name = result[0]
                        recognized_info += f"Student Name: {student_name[:5]}, Count: {count}\n"

                # Show the student information only in the separate window
                if recognized_info:
                    recognized_img = np.zeros((650, 250, 3), dtype=np.uint8)
                    recognized_info_lines = recognized_info.split('\n')  # Split the lines
                    y_offset = 35  # Initial y-offset for text
                    for info_line in recognized_info_lines:
                        cv2.putText(recognized_img, info_line, (10, y_offset), cv2.FONT_HERSHEY_PLAIN, 0.9, (0, 255, 0), 1)
                        y_offset += 15  # Increment y-offset for next line
